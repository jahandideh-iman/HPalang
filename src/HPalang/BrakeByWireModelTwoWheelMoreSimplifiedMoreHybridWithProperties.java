 ...
 */
public class BrakeByWireModelTwoWheelMoreSimplifiedMoreHybridWithProperties
{
    public static final boolean ADD_TIME_PROPERTY_MONITOR = false;
    public static final boolean ADD_SAFETY_PROPERTY_MONITOR = false;
    public static final int WHEEL_INDEX_FOR_APPLYING_PROPERTY = 0;
    ...
    public static final String Brake__holding_brake = "holding";
    ...
    public static ModelDefinition Create()
    {
        ...
        {
            PhysicalActor timeMonitor = new PhysicalActor("monitor", monitorType, 1);
            ...
            Mode brakeConstantMode = brakeType.FindMode(Brake__increasing_brake);
            ...
            if(WHEEL_INDEX_FOR_APPLYING_PROPERTY == 0)
            {
                wheel_FR.BindInstance(wheelParam, timeMonitor, CommunicationType.Wire);
                wheel_FL.BindToEmpty(wheelParam);
            }
            else
            {
                wheel_FR.BindToEmpty(wheelParam);
                wheel_FL.BindInstance(wheelParam, timeMonitor, CommunicationType.Wire);
            }
        ...
        {
            PhysicalActor timeMonitor = new PhysicalActor("sfMntr", monitorType, 2);
            //PhysicalActor timeMonitorFL = new PhysicalActor("sfMntrFL", monitorType, 2);
            
            InstanceParameter wheelControllerParam = new InstanceParameter("monitor", monitorType);
            ...
            InstanceParameter wheelParam = new InstanceParameter("mntr", monitorType);
            wheelType.AddInstanceParameter(wheelParam);
            MessageHandler wheelSetTorqueHandler = wheelType.FindMessageHandler(Wheel__torque_port);
            wheelSetTorqueHandler.AddStatement(CreateSendStatement(wheelParam, monitorType.FindMessageHandler(Time_Monitor__stop)));
            ...
            if(WHEEL_INDEX_FOR_APPLYING_PROPERTY == 0)
            {
                ...
                wheel_controller_FL.BindToEmpty(wheelControllerParam);
                wheel_FL.BindToEmpty(wheelParam);
            }
            else
            {
                wheel_controller_FR.BindToEmpty(wheelControllerParam);
                wheel_FR.BindToEmpty(wheelParam);
                
                wheel_controller_FL.BindInstance(wheelControllerParam, timeMonitor, CommunicationType.Wire);
                wheel_FL.BindInstance(wheelParam, timeMonitor, CommunicationType.Wire);
            }
            //wheel_controller_FL.BindInstance(wheelControllerParam, timeMonitorFL, CommunicationType.Wire);
    ...
    private static void FillFleshForWheelType(PhysicalActorType wheelType, MessageHandler wheel_speed_port)
    {
        ...
        brakeMode.SetInvarient(new Invarient(CreateBinaryExpression(
                CreateBinaryExpression(timer, "<=", Const(Wheel__period_const)),
                "&&",
                CreateBinaryExpression(speed, ">=", Const(0f)))));
        
        brakeMode.SetGuard(CreateGuard(
                CreateBinaryExpression(timer, "==", Const(Wheel__period_const)),
                "||",
                CreateBinaryExpression(speed, "==", Const(0f))));
    ...
    private static void FillSkeletonForBrakeType(PhysicalActorType brakeType, SoftwareActorType globalBrakeControllerType)
    {
        ...
        brakeType.AddMode(new Mode(Brake__holding_brake));
    ...
    private static void FillFleshForBrakeType(PhysicalActorType brakeType, MessageHandler brakePercentPort)
    {
        ...
        Mode increasing = brakeType.FindMode(Brake__increasing_brake);
        Mode holding = brakeType.FindMode(Brake__holding_brake);
                
        increasing.SetInvarient(new Invarient(
                CreateBinaryExpression(
                        CreateBinaryExpression(timer, "<=", Const(Brake__period_const)),
                        "&&",
                        CreateBinaryExpression(
                                VariableExpression(brake_percent), 
                                "<=", 
                                VariableExpression(max_brake_percent)))));
        
        increasing.SetGuard(new Guard(CreateBinaryExpression(
                CreateBinaryExpression(timer, "==", Const(Brake__period_const)),
                "||",
                CreateBinaryExpression(
                        VariableExpression(brake_percent), 
        ...
        increasing.AddDifferentialEquation(new DifferentialEquation(timer, Const(1f)));
        increasing.AddDifferentialEquation(new DifferentialEquation(brake_percent, Const(1f)));
        
        increasing.AddAction(new IfStatement(
                CreateBinaryExpression(brake_percent,">=",VariableExpression(max_brake_percent)),
                Statement.StatementsFrom(CreateModeChangeStatement(holding)), 
                Statement.StatementsFrom(CreateModeChangeStatement(increasing))
        ));
        increasing.AddAction(CreateResetFor(timer)); 
        increasing.AddAction(CreateSendStatement(controller, brakePercentPort, VariableExpression(brake_percent)));
        
        holding.SetInvarient(new Invarient(
                CreateBinaryExpression(timer, "<=", Const(Brake__period_const))));
        
        holding.SetGuard(new Guard(
                CreateBinaryExpression(timer, "==", Const(Brake__period_const))));
        
        holding.AddDifferentialEquation(new DifferentialEquation(timer, Const(1f)));
        holding.AddDifferentialEquation(new DifferentialEquation(brake_percent, Const(0f)));
        
        holding.AddAction(CreateModeChangeStatement(holding));
        holding.AddAction(CreateResetFor(timer)); 
        holding.AddAction(CreateSendStatement(controller, brakePercentPort, VariableExpression(brake_percent)));
